...
export async function fixTopField(typ: "local" | "remote", db: string, isInfoExistResult: boolean) {
    if (!isInfoExistResult) {
        ...
        if (result[0].results.length === 0) {
            ...
            await $`bunx wrangler d1 execute ${db}  --${typ} --json --command "ALTER TABLE feeds ADD COLUMN top INTEGER DEFAULT 0"`
                .quiet();
        } else {
...
export async function updateMigrationVersion(typ: "local" | "remote", db: string, version: number) {
    ...
    await $`bunx wrangler d1 execute ${db}  --${typ} --json --command "UPDATE info SET value='${version}' WHERE key='migration_version'"`
        .quiet();
