...
export function WritingPage({ id }: { id?: number }) {
    ...
    const [categories, setCategories] = cache.useCache("categories", "");
    ...
    function publishButton() {
        ...
        const tagsplit = tags
            ...
            .filter((tag) => tag !== "")
            .map((tag) => tag.trim()) || [];
        const categoriessplit = categories
            .split("/")
                ...
                .then(({ data }) => {
                    if (data && typeof data !== "string") {
                        ...
                        if (categories == "" && data.categories) {
                            setCategories(data.categories.map(({ name }) => `/${name}`).join(" "));
                        }
                        if (tags == "" && data.tags) setTags(data.tags.map(({ name }) => `#${name}`).join(" "));
                        if (alias == "" && data.alias) setAlias(data.alias);
                        if (content == "") setContent(data.content);
                        if (summary == "") setSummary(data.summary);
                        if (status === "publish") {
                            setStatus(data.status as "publish" | "draft" | "private" || "publish");
                        }
                ...
                <div className={className}>
                    ...
                    <div className="select-none flex flex-row justify-between items-center mt-6 mb-2 px-4">
                        ...
                        <select
                            className="border-solid rounded"
                            value={property}
                            onChange={(e) => setProperty(e.target.value as "post" | "page")}
                        >
                    ...
                    <div className="select-none flex flex-row justify-between items-center mt-4 mb-2 px-4">
                        ...
                        <Calendar
                            value={updatedAt}
                            onChange={(e) => setUpdatedAt(e.value || undefined)}
                            showTime
                            touchUI
                            hourFormat="24"
                        />
                    ...
                    <div className="bg-w rounded-2xl shadow-xl shadow-light p-4">
                        ...
                        <div className="flex flex-col mx-4 my-2 md:mx-0 md:my-0 gap-2">
                            ...
                            <div className={`grid grid-cols-1 ${preview === "comparison" ? "sm:grid-cols-2" : ""}`}>
                                ...
                                <div
                                    className={"px-4 h-[600px] overflow-y-scroll "
                                        + (preview !== "edit" ? "" : "hidden")}
                                >
                                    <Markdown
                                        content={content ? content : "> No content now. Write on the left side."}
                                    />
                                </div>
                            </div>
                        </div>
