...
async function publish(
    { name, avatar, desc, url, showAlert }: {
        name: string;
        avatar: string;
        desc: string;
        url: string;
...
export function FriendsPage() {
    ...
    useEffect(() => {
        ...
        client.friend.index.get({
            ...
            if (data) {
                const friends_available = data.friend_list?.filter(({ health, accepted }) =>
                    health.length === 0 && accepted === 1
                ) || [];
                shuffleArray(friends_available);
                setFriendsAvailable(friends_available);
                const friends_unavailable = data.friend_list?.filter(({ health, accepted }) =>
                    health.length > 0 && accepted === 1
                ) || [];
            ...
            <Waiting for={friendsAvailable.length !== 0 || friendsUnavailable.length !== 0 || status === "idle"}>
                <main className="w-full flex flex-col justify-center items-center mb-8 t-primary ani-show">
                    ...
                    {profile && (profile.permission || config.get("friend_apply_enable"))
                        && (
                            <div className="wauto t-primary flex text-start text-2xl font-medium mt-8">
