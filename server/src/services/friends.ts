        ...
        .group("/friend", (group) =>
            group.get("/", async ({ admin, uid }) => {
                const friend_list = await (admin
                    ...
                    : db.query.friends.findMany({ where: eq(friends.accepted, 1) }));
                ...
                .put(
                    "/:id",
                    async (
                        { admin, uid, username, set, params: { id }, body: { name, desc, avatar, url, accepted } },
                    ) => {
                        const config = ClientConfig();
                        const enable = await config.getOrDefault("friend_apply_enable", true);
                        if (!enable && !admin) {
...
export async function friendCrontab(env: Env, ctx: ExecutionContext) {
    ...
    for (const friend of friend_list) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                ctx.waitUntil(
                    db.update(schema.friends).set({ health: `${response.status}` }).where(
                        eq(schema.friends.id, friend.id),
                    ),
                );
                unhealthy++;
            }
    ...
    console.info(
        `update friends health done. Total: ${health + unhealthy}, Healthy: ${health}, Unhealthy: ${unhealthy}`,
    );
}
